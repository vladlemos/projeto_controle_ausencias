...
<body>
	...
	<form action="registraSolicitacaoFerias.php" method="post">
		...
		<fieldset>
			<legend> SOLICITAR AUSÊNCIA</legend>
			
			<label>PERIODO AQUISITIVO: 
				<input type="text" 
						name="periodoAquisitivo" 
						value="<?= date("d/m/Y", strtotime($ferias->getPeriodoAquisitivo())); ?>"
						size="10" 
						readonly>
			</label>
			...
			<label>DATA INICIO: 
				<input type="text" 
					   id="datePickerDataInicio" 
					   required>
				<input type="hidden" id="alternate" name="dataInicio"></label>


			<label>DATA RETORNO: 
				<input type="text"
					   id="datePickerDataRetorno" 
					   required>
				<input type="hidden" id="alternate2" name="dataRetorno"><br></label>
			...
			<label>QUANTIDADE DIAS ABONO: 
				<input type="number" name="quantidadeDiasAbono" min="0" max= "10" required><br>
			</label>

			<label>QUANTIDADE DE PARCELAS:
				 <input type="number" name="quantidadeParcelas" min=0 max=10 required><br>
			</label>
